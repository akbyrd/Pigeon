...
auto
CreateNotificationWindow = [](NotificationWindow& notification, HINSTANCE hInstance)
{
...
*/
auto
EnforceSingleInstance = [](NotificationWindow& notification, u64& startTime, u32& WM_NEWINSTANCE, HANDLE& singleInstanceMutex)
{
...
auto
RegisterHotkeys = [](NotificationWindow& notification, Hotkey* hotkeys, u8 hotkeyCount, HANDLE singleInstanceMutex)
{
...
auto
Misc = [](NotificationWindow& notification)
{
...
auto
MessageLoop = [](NotificationWindow& notification, u64 startTime, u32 WM_NEWINSTANCE, Hotkey* hotkeys, u8 hotkeyCount, HANDLE singleInstanceMutex)
{
	MSG msg = {};
	b32 quit = false;
...
wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PWSTR pCmdLine, i32 nCmdShow)
{
	...
	if (CreateNotificationWindow(notification, hInstance))
	{
		...
		{
			if (RegisterHotkeys(notification, hotkeys, ArrayCount(hotkeys), singleInstanceMutex))
			{
				if (InitializeAudio(&notification))
				{
					Misc(notification);
					returnValue = MessageLoop(notification, startTime, WM_NEWINSTANCE, hotkeys, ArrayCount(hotkeys), singleInstanceMutex);
				}
				TeardownAudio();
			}
		}
